...
for df in [ad, aps, census, covertype, heart]
    ...
    grid = IAI.GridSearch(lnr,
        max_depth=4:10,
        minbucket=5,
    	hyperplane_config=(sparsity=3,),
    	fast_num_support_restarts = 1
    )

    time_to_run = @elapsed IAI.fit!(grid, train_X_sparse, train_y, validation_criterion=:auc)
    println("Time to run OCT-H with sparse feature selection for ",  dfnames[i], ": ", time_to_run)
    ...
    IAI.write_html(dfnames[i] * "Sparse_OCT-H_Tree.html", IAI.get_learner(grid))
    ...
    println("Time to run full OCT-H for ",  dfnames[i], ": ", time_to_run)

    println("Full tree AUC for ",  dfnames[i], ": ", IAI.score(grid2, test_X, test_y, criterion=:auc))

    # write the tree visualization to a file
    IAI.write_html(dfnames[i] * "OCT-H_Tree.html", IAI.get_learner(grid2))
